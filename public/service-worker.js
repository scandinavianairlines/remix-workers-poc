...
var require_errors = __commonJS({
  "node_modules/@remix-run/server-runtime/dist/errors.js"(exports) {
    ...
    function sanitizeError(error, serverMode2) {
      if (error instanceof Error && serverMode2 !== mode.ServerMode.Development) {
    ...
    function sanitizeErrors(errors, serverMode2) {
      return Object.entries(errors).reduce((acc, [routeId, error]) => {
        return Object.assign(acc, {
          [routeId]: sanitizeError(error, serverMode2)
        });
      }, {});
    }
    function serializeError2(error, serverMode2) {
      let sanitized = sanitizeError(error, serverMode2);
      return {
        message: sanitized.message,
        stack: sanitized.stack
      };
    }
    function serializeErrors(errors, serverMode2) {
      if (!errors)
        return null;
      ...
      for (let [key, val] of entries) {
        if (router.isRouteErrorResponse(val)) {
          ...
          let sanitized = sanitizeError(val, serverMode2);
          serialized[key] = {
            message: sanitized.message,
            stack: sanitized.stack,
            __type: "Error",
...
var require_responses = __commonJS({
  "node_modules/@remix-run/server-runtime/dist/responses.js"(exports) {
    ...
    function createDeferredReadableStream2(deferredData, signal, serverMode2) {
      ...
      let stream = new ReadableStream({
        async start(controller) {
          ...
          for (let preresolvedKey of preresolvedKeys) {
            enqueueTrackedPromise(controller, encoder, preresolvedKey, deferredData.data[preresolvedKey], serverMode2);
          }
          let unsubscribe = deferredData.subscribe((aborted, settledKey) => {
            if (settledKey) {
              enqueueTrackedPromise(controller, encoder, settledKey, deferredData.data[settledKey], serverMode2);
            }
          });
    ...
    function enqueueTrackedPromise(controller, encoder, settledKey, promise, serverMode2) {
      if ("_error" in promise) {
        controller.enqueue(encoder.encode("error:" + JSON.stringify({
          [settledKey]: promise._error instanceof Error ? errors.serializeError(promise._error, serverMode2) : promise._error
        }) + "\n\n"));
      } else {
...
// routes-module:routes/_app.flights.jsx?worker
...
// routes-module:routes/selection.jsx?worker
...
var routes = [
  ...
  { file: "routes/_app.flights.jsx", path: "flights", module: app_flights_exports, id: "routes/_app.flights", parentId: "routes/_app" },
  { file: "routes/selection.jsx", path: "selection", module: selection_exports, id: "routes/selection", parentId: "root" },
  { file: "routes/_index.jsx", path: "undefined", module: route3, id: "routes/_index", parentId: "root" },
  { file: "routes/_app.jsx", path: "undefined", module: app_exports, id: "routes/_app", parentId: "root" }
];
...
// scripts/utils/response.js
var import_responses = __toESM(require_responses());
...
function handleRequest({
  ...
  const route = routes2.find((route2) => route2.id === _data);
  try {
...
function _errorHandler({ error, handler: handleError }) {
  ...
  return (0, import_responses2.json)(serializeError(errorInstance, serverMode), {
    status: 500,
    headers: {
      "X-Remix-Error": "yes"
    }
